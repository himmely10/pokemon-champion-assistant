...
def test_missing_statistics_keeps_learnset_and_no_mega_inheritance(tmp_path):
    ...
    assert damage and status and "暂无双打采用率" in notice
    assert all(m["usage_percent"] is None for m in damage + status)
