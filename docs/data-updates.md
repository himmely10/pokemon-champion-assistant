...
普通用户通过软件更新中心导入维护者提供的 ZIP，或在配置了真实 HTTPS 渠道后检查下载。一次分析固定静态资料、采用率和规则的组合；新包完成校验后在下次分析使用，已有弹窗继续使用旧快照。软件关闭后不执行检查，也不创建 Codex 自动化。

下文的联网采集命令是维护者入口，继续保留。发行给用户前执行：

```powershell
.\.venv-ui\Scripts\python.exe -X utf8 scripts/build_data_bundle.py --data-dir pokemon --output-dir artifacts/data-dist
```

脚本会把旧 JSON 输入转换为分版本 `catalog.sqlite` 和 `usage.sqlite`，保留公共 JSON 证据、普通／闪光图标及字段缺失语义，生成包含逐文件 SHA-256 的 ZIP，然后执行软件真实导入验证。当前支持双打，不能把旧赛季统计重新标为新赛季。没有在线地址时，ZIP 仍可离线导入；软件应显示“渠道未配置”。

输出、CI、无变化跳过和发布顺序详见 [资料与程序发行指南](release.md)。`current.json` 是旧组件入口；软件分析以原子写入的 `release.json` 组合描述为准。维护者命令改变组件时，不强行修改已打开分析的快照。
...
维护者采集的组件需要构建成资料包并通过软件更新服务导入；下次分析使用新版，已打开的资料台及其识别器继续使用原来的同一组合版本。
...
```text
pokemon/
  release.json                  # v0.2 分析权威入口：静态、采用率、规则的完整组合
  current.json                  # 旧静态组件入口，维护者采集器继续使用
  index.json                    # 派生兼容索引，directory 指向版本内真实目录
  _versions/<包ID>/
    manifest.json               # 每个已发布文件的 SHA-256
    catalog.sqlite              # v0.2 静态领域查询权威；旧包可仅有 JSON
  ...
  _reports/                     # 每次操作报告
  _usage/snapshots/              # 不可变 JSON 证据与 usage.sqlite 统计快照
  _staging/                     # 尚未发布的暂存；失败残留不参与读取
  妙蛙花/ ...                   # 原平铺文件保留为历史原件，不再表示最新数据
```

维护者和旧兼容入口通过 `resolve_dataset(data_dir)` 获取静态组件。v0.2 桌面分析使用 `SnapshotManager` 从 `release.json` 同时固定静态与采用率，既有识别器保持原图标缓存。根目录兼容索引可供简单脚本查看，但不能作为桌面分析组合入口；若采集发布时进程在兼容导出前中断，再次 `sync` 可修复该派生文件。
...
维护者组件更新写入暂存区，整个组件完成后才原子切换对应 `current.json`；首次升级会先保存原目录的可回滚包。软件导入则校验静态与采用率后一次切换 `release.json`，不让分析拼接独立变化的组件。发布与回滚受进程锁保护，程序退出后锁由操作系统释放。校验清单拒绝绝对路径、目录穿越和链接逃逸；抓取的页面只解析为数据，不执行脚本。


安装新版程序后，可在更新中心点击“采用当前软件附带的离线资料”，主动切换到新安装包附带的资料。该操作不会自动覆盖可能更新的在线资料，也不修改个人队伍；如需撤销，可恢复上一版。线上渠道版本必须与包内版本一致，否则拒绝切换。大资料包流式下载到临时文件并增量校验摘要。
